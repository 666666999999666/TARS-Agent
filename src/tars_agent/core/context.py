...
class ExecutionContext:
    ...
    def add_assistant_message(self, content: list[Any]) -> None:
        ...
        self.audit_messages.append({"role": "assistant", "content": list(content)})
    ...
    def add_tool_result(
        ...
        if (
            ...
            audit_last = self.audit_messages[-1]
            assert isinstance(audit_last["content"], list)
            audit_last["content"].append(dict(block))
        else:
            self.messages.append({"role": "user", "content": [block]})
            self.audit_messages.append({"role": "user", "content": [dict(block)]})
