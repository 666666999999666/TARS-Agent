...
class Compactor:
    ...
    async def compact(
        ...
        *,
        ...
        context.compactions.append(result)
        if publish:
            self.write_summary(result.summary_text)
    ...
    async def compact_messages(
        ...
        if response.stop_reason != "end_turn" or response.tool_calls:
            logger.warning("compactor: incomplete model response; retaining successful history")
            return None
