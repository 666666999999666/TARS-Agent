...
def _terminal_failure_reason(reason: str) -> str:
    if reason in ("max_tokens", "refusal", "stop_sequence", "pause_turn"):
        return reason
    return f"unexpected_stop_reason:{reason or 'none'}"
        ...
        while not context.is_done():
            ...
            try:
                ...
                if exc.partial_text:
                    ...
                    # assistant message. RuntimeService leaves failed Run messages
                    # uncommitted, so partial output never enters formal context.
                    context.audit_messages.append(
                        {
                            "role": "assistant",
                            "content": [
                                {
            ...
            if response.stop_reason == "end_turn" and (
                not response.text.strip() or response.tool_calls
            ):
                context.mark_failed("llm_protocol_error")
                break
            ...
            if response.stop_reason == "end_turn":
                ...
                context.mark_failed(_terminal_failure_reason(response.stop_reason))
