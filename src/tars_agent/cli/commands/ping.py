...
from tars_agent.core.bus.envelope import JsonRpcError, JsonRpcSuccess
from tars_agent.core.config import TarsConfig


# 同步入口：运行 ping 协程，连接失败时打印错误并退出
def cmd_ping(config: TarsConfig) -> None:
    try:
...
async def _ping(config: TarsConfig) -> None:
    t0 = time.monotonic()
    ...
    req = {
        ...
        "id": "cli-1",
        "method": "core.ping",
        "params": {"client": f"cli/{tars_agent.__version__}"},
    }
