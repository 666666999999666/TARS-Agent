#!/usr/bin/env python3
"""Generate WIRE_PROTOCOL.md from pydantic models in tars_agent.core.bus."""
...
from tars_agent.core.bus.envelope import EventOverflowEnvelope, EventPushEnvelope
...
def generate() -> str:
    ...
    agent_run_req_example = {
        ...
        "id": "u-2",
        "method": "agent.run",
        "params": {"goal": "总结 README.md 的主要章节", "workspace_root": "/workspace"},
    }
    ...
    session_create_req_example = {
        ...
        "params": {"mode": "chat", "title": "", "workspace_root": "/workspace"},
    }
    ...
    session_send_resp_example = {
        "jsonrpc": "2.0",
        "id": "u-5",
        "result": {"run_id": run_id, "status": "queued", "deduplicated": False},
    }
    event_push_example = {
        "kind": "event",
        "protocol_version": 2,
    ...
    sections = [
        ...
        "\n",
        _model_section("CoreShutdownCommand", CoreShutdownCommand),
        "\n",
        _model_section("CoreShutdownResult", CoreShutdownResult),
        "\n",
        ...
        "\n",
        _model_section("SessionInfo", SessionInfo),
        "\n",
        ...
        "\n",
        _model_section("SessionGetCommand", SessionGetCommand),
        "\n",
        _model_section("SessionGetResult", SessionGetResult),
        "\n",
        ...
        "\n",
        _model_section("SessionRetryCommand", SessionRetryCommand),
        "\n",
        _model_section("SessionRetryResult", SessionRetryResult),
        "\n",
        _model_section("RunGetCommand", RunGetCommand),
        "\n",
        _model_section("RunCancelCommand", RunCancelCommand),
        "\n",
        _model_section("RunInfo", RunInfo),
        "\n",
        ...
        "\n",
        _model_section("McpStatusCommand", McpStatusCommand),
        "\n",
        _model_section("McpServerInfo", McpServerInfo),
        "\n",
        _model_section("McpStatusResult", McpStatusResult),
        "\n## Server Push\n\n",
        ...
        "\n",
        _model_section("EventOverflowEnvelope", EventOverflowEnvelope),
        "\n## IPC Events\n\n",
        "Events sent over the IPC socket (daemon → client).\n\n",
        _model_section("CoreStartedEvent", CoreStartedEvent),
        "\n## Run Events\n\n",
        "Run events are persisted in the SQLite `events` table by `DurableEventHub` before "
        "they are forwarded over IPC to subscribed clients.\n\n",
        ...
        "\n",
        _model_section("ToolExecutionStartedEvent", ToolExecutionStartedEvent,
            {"type": "tool.execution_started", "run_id": run_id,
             "tool_use_id": "toolu_01", "tool_name": "read_file", "attempt": 1,
             "ts": ts}),
        "\n",
        ...
        _model_section("LogLineEvent", LogLineEvent,
            {"type": "log.line", "run_id": run_id, "level": "INFO",
             "source": "tars_agent.core.loop", "message": "step 1 started", "ts": ts}),
        "\n## Subagent Events\n\n",
