...
async def test_trace_observer_failure_never_changes_model_result(tmp_path: Path) -> None:
    writer = TraceWriter(tmp_path / "trace.jsonl")

    def failing_emit(record: TraceRecord) -> None:
        raise OSError("observer failed")

    writer.emit = failing_emit  # type: ignore[method-assign]
    inner = AsyncMock()
    inner.chat = AsyncMock(return_value=_make_response())
    result = await TracingProvider(inner, writer).chat([], [], EventBus(), "run")
