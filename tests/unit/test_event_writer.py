...
from tars_agent.core.bus.events import RunFinishedEvent, RunStartedEvent
...
async def test_event_writer_creates_parent_dirs(tmp_path: Path) -> None:
    ...
    async with EventWriter(path, run_id="abc123") as writer:
        await writer.handle(event)
...
async def test_event_writer_handle_when_not_open_is_noop(tmp_path: Path) -> None:
    writer = EventWriter(tmp_path / "events.jsonl", run_id="r1")
    event = RunStartedEvent(run_id="r1", goal="g", ts="2026-05-11T00:00:00Z")
...
async def test_event_writer_oserror_is_logged(
    ...
    with caplog.at_level(logging.ERROR, logger="tars_agent.core.events.writer"):
        async with EventWriter(path, run_id="r1") as writer:
...
async def test_shared_bus_writers_only_record_their_own_run(tmp_path: Path) -> None:
    bus = EventBus()
    first = tmp_path / "first.jsonl"
    second = tmp_path / "second.jsonl"
    async with EventWriter(first, run_id="first") as first_writer:
        async with EventWriter(second, run_id="second") as second_writer:
            first_writer.subscribe(bus)
            second_writer.subscribe(bus)
            for run_id in ("first", "second"):
                await bus.publish(RunStartedEvent(run_id=run_id, goal=run_id, ts="now"))
                await bus.publish(RunFinishedEvent(
                    run_id=run_id, status="success", steps=1, ts="later",
                ))
    for path, run_id in ((first, "first"), (second, "second")):
        rows = [json.loads(line) for line in path.read_text().splitlines()]
        assert [row["type"] for row in rows] == ["run.started", "run.finished"]
