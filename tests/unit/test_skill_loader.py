...
from tars_agent.core.skills.loader import Skill, SkillLoader
...
def test_frontmatter_parsed(tmp_path: Path) -> None:
    from tars_agent.core.skills.loader import _parse_skill_file
...
# 设计：在 .tars/skills/ 中写入同名文件，用 monkeypatch 修改 cwd，断言加载到的是本地版本
def test_project_overrides_global(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    local_skills = tmp_path / ".tars" / "skills"
...
@pytest.mark.parametrize("name", ["../private", "..\\private", "C:secret", "CON", "a/b"])
def test_skill_resource_name_cannot_escape(name: str) -> None:
    with pytest.raises(ValueError):
        SkillLoader().resolve(name)


def test_global_skills_use_explicit_new_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    old = tmp_path / "user"
    for dirname in (".kama", ".tars"):
        legacy = old / dirname / "skills"
        legacy.mkdir(parents=True)
        (legacy / "oldonly.md").write_text("old private sentinel", encoding="utf-8")
    selected = tmp_path / "new-home"
    (selected / "skills").mkdir(parents=True)
    (selected / "skills" / "selected.md").write_text("new home skill", encoding="utf-8")
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    monkeypatch.setattr(Path, "home", lambda: old)
    monkeypatch.setenv("TARS_HOME", str(selected))
    monkeypatch.chdir(workspace)
    loader = SkillLoader()
    assert loader.resolve("oldonly") is None
    assert "oldonly" not in loader.list_all()
    assert loader.resolve("selected") is not None
