...
from tars_agent.core.compact.compactor import Compactor
from tars_agent.core.context import ExecutionContext
from tars_agent.core.events.bus import EventBus
from tars_agent.core.llm.provider import LlmStreamInterruptedError
...
async def test_high_context_usage_compacts_before_next_model_step(
    tmp_path: Path,
) -> None:
    provider = _MockProvider(
        [
            ...
            LlmResponse(
                ...
                usage=UsageStats(input_tokens=100, output_tokens=12),
            ),
...
async def test_interrupted_stream_preserves_partial_text_for_audit_only() -> None:
    provider = _MockProvider(
        [],
        exc=LlmStreamInterruptedError(
    ...
    assert ctx.status == "failed"
    assert ctx.reason == "llm_stream_interrupted"
    assert ctx.audit_messages == [
        {
            "role": "assistant",
            "content": [
                {"type": "text", "text": "partial answer", "partial": True}
            ],
        }
    ]
...
@pytest.mark.parametrize("stop_reason", ["max_tokens", "refusal", "pause_turn", "unknown"])
async def test_terminal_reason_stops_after_one_step(stop_reason: str) -> None:
    provider = _MockProvider([LlmResponse(stop_reason=stop_reason, text="incomplete")])
    registry = ToolRegistry()
    context = ExecutionContext(run_id="stop-reason", goal="test", max_steps=5)
    await AgentLoop(provider, registry, EventBus()).run(context)
    assert context.status == "failed"
    assert context.step == 1
    assert context.reason == (stop_reason if stop_reason != "unknown" else "unexpected_stop_reason:unknown")
