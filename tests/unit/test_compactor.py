...
@pytest.mark.asyncio
@pytest.mark.parametrize("reason", ["max_tokens", "refusal", "unknown"])
async def test_non_successful_summary_preserves_committed_history(tmp_path: Path, reason: str) -> None:
    provider = _stub_provider("A non-empty but incomplete summary")
    provider.chat.return_value.stop_reason = reason
    compactor = Compactor(EventBus(), tmp_path, "session-safe")
    context = ExecutionContext(run_id="r-safe", goal="test", max_steps=2)
    messages = _make_messages(2)
    context.messages = list(messages)
    result = await compactor.compact(context, provider)
    assert result is None
